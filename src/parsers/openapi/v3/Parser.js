...
import tv4 from 'tv4'
import openapiSchema from '../../../../schemas/openapi/schema.json'
...
export const __errors__ = {
  ...
  NotAnOpenApiV3: class NotAnOpenApiV3Error extends TypeError {}
}
...
/**
 * A Parser that converts openapi v3.0 formatted objects into Api Records
 */
 ...
 * @returns {void}
 */
methods.handleInvalidopenapi = () => {
  const message = 'Invalid openapi File (invalid schema / version < 3.0)\n' +
    tv4.error + '\n' +
    'Potential culprit: ' + tv4.error.dataPath
  const error = new __errors__.NotAnOpenApiV3(message)
  throw error
}
...
/**
 * gives a confidence score that the string passed is an openapi file
...
/**
 * suggests an API name based on the content of an openapi file
 * @param {string} content: the content of the openapi file
...
/**
 * tests whether an object is an openapi object or not
 * @param {Object} maybeopenapi: the object to test
 * @returns {boolean} true if it is an openapi, false otherwise
 */
methods.isopenapi = (maybeopenapi) => {
  const isValid = tv4.validate(maybeopenapi, openapiSchema)
  return isValid
}

/**
 * converts an openapi Contact Object into a Contact Record
...
/**
 * converts an openapi License Object into a License Record
...
/**
 * convert an openapi Info Object into an Info Record
...
/**
 * Fixes the openapi file to not rely on an external context, like the url we got the file from
 * @param {openapiObject} openapi: the openapi file that may need fixing
 * @param {string?} url: the context to fix the openapi file
 * @returns {openapiObject}: the fixed openapi
 *
 * TODO: this may raise reference issue when dealing with silently defined local files. we could
 * fix this.
 */
methods.fixExternalContextDependencies = (openapi, url) => {
  if (!openapi.servers) {
    openapi.servers = { url: url ? url : 'localhost' }
  }
...
/**
 * extracts entries of an openapi Resource Object and returns the ones that are methods
 * @param {openapiResourceObject} resource: the openapi resource to extract the method entries of
 * @returns {Array<Entry<string, openapiOperationObject>>} the methods entries
 */
...
/**
 * separates parameters and references from a list of openapi Parameter Objects
 * @param {Array<openapiParameterObject|openapiReferenceObject>} params: the list of parameters to
 * separate.
 * @returns {
 *   {
 *     parameters: Array<Entry<string, openapiParameterObject>>,
 *     references: Array<Entry<string, openapiReferenceObject>>
 *   }
...
/**
 * converts an openapi Reference Object into into an Entry of a Reference Record.
 * @param {string} type: the type of the reference
 * @param {string} key: the URI of the openapi Reference Object
...
/**
 * transforms a list of openapi Reference Objects into a map of Reference Records
 * @param {string} type: the type of the references
 * @param {Array<string>} refs: the array of openapi Reference Objects
 ...
 * Entry.
 * @param {Array<openapiParameterObject|openapiReferenceObject>} params: the shared parameters
 * @param {string} key: the key of the Entry.
 * @param {openapiOperationObject} value: the openapi Operation Object to update.
 * @returns {Entry<string, openapiOperationObject>} the updated operation object
 */
 ...
 * @param {openapiOperationObject} operation: the operation object to get the consumes field of
 * @returns {
 *   {
 ...
 * @param {Store} store: the Store Record that holds shared values.
 * @param {openapiOperationObject} operation: the operation object to get the produces field of
 * @returns {
 *   {
...
/**
 * converts an openapi location (for the `in` field of a parameter object) into a location in a
 * ParameterContainer.
 ...
 * @param {Array<openapiSecurityRequirementObject>} requirements: the list of security requirements.
 ...
 * @param {openapiOperationObject} operation: the operation object in which there may be a consumes
 * field.
 ...
 * @param {openapiResponseObject} value: the response object to update.
 * @returns {Entry<string, openapiResponseObject>} the updated response object, as an Entry.
 */
 ...
 * @param {openapiOperationObject} operation: the operation object that holds the schemes defined
 * for this operation.
 ...
 * @param {openapiOperationObject} operation: the operation object to extract the parameters from
 ...
 * field.
 * @param {Store} store: the store from which to get the shared responses
 * @param {openapiOperationObject} operation: the openapi operation object to extract the responses
 * from.
 ...
 * @param {openapiOperationObject} operation: the openapi operation object to extract the endpoints
 * from.
...
/**
 * extracts the request Id from an openapi operation object. It maps to an operationId if it exists.
...
/**
 * converts an openapi Operation Object into a Request Record.
 * @param {Store} store: the store from which to get shared values that may be used by the Request.
 * @param {Array<openapiSecurityRequirementObject>} security: the global security requirements
 * @param {string} key: the method associated with this openapi Operation.
 * @param {openapiOperationObject} value: the operation object to convert.
 ...
 * @param {Array<openapiSecurityRequirementObject>} security: the global security requirements
 * @param {openapiResourceObject} resourceObject: the resource object to extract the requests from
...
/**
 * updates the path endpoint from the Resource with pathParameters from its openapi operations
...
/**
 * converts an openapi Resource Object into a Resource Record.
 * @param {Store} store: the store from which to get the possibly shared resources relevant to this
 * Resource.
 * @param {Array<openapiSecurityRequirementObject>} security: the global security requirements
 * @param {openapiPathObject} paths: the paths of an openapi object.
 * @param {string} path: the path of the openapi resource object.
 * @returns {Resource} the corresponding Resource.
 *
 * FIXME: we assume all external references are resolved.
 * TODO: support $ref for openapi path Items (i.e. this can be a $ref)
...
/**
 * converts the openapi path object into a map of Resources.
 * @param {Store} shared: the store in which shared objects are stored.
 * @param {openapiObject} openapi: the openapi file to conver the path objects from.
...
/**
 * returns the list of shared endpoints in the openapi file (in v3, it's the server.urls)
 ...
 * @param {openapiParameterObject} param: the openapi parameter object we are extracting constraints
 * from.
 ...
 * @param {openapiParameterObject} parameter: the parameter to extract the constraints of
...
/**
 * extracts applicableContexts from an openapiParameter location value
 * @param {string} location: the location of an openapi parameter
...
/* eslint-disable max-statements */
/**
 * converts an openapi parameter object into a Parameter Object
 * @param {Entry<string, openapiParameterObject>} parameterEntry: the entry to convert
...
/**
 * converts a List of entries of openapi parameters into a map of Parameters using the entry key
 * as keys in the map.
 * @param {List<Entry<string, openapiParameterObject>>} parameters: the list of parameters to
...
/**
 * extracts all globally shared Parameters in the openapi file. This checks for the consumes and
 ...
 * @param {Object<string, openapiParameterObject>} parameters: the content of the prameters field,
 * if it exists.
...
/**
 * creates a ParameterContainer from an openapi Response Object
 ...
 * @param {string} key: the key of the Entry
 * @param {openapiResponseObject} value: the response object to convert
...
/**
 * converts an openapi Response Definitions Object into a Map of Responses.
 * @param {openapiDefinitionsResponseObject} responses: a dictionary of responses to convert
 ...
 * @param {openapiAuthObject} auth: the auth to find the type of
...
/**
 * converts an openapi Security Definition into a BasicAuth, including the potential interface it
 * implements.
...
/**
 * converts an openapi Security Definition into a ApiKeyAuth, including the potential interface it
 * implements.
 * @param {Interface?} interfaceUsingAuth: the Interface to apply, if it exists
 * @param {string} key: the key of the entry. Used to save the auth name
 * @param {openapiSecurityDefinition} auth: the auth to convert
...
/**
 * converts an openapi Security Definition into an OAuth2Auth, including the potential interface it
 * implements.
 * @param {Interface?} interfaceUsingAuth: the Interface to apply, if it exists
 * @param {string} key: the key of the entry. Used to save the auth name
 * @param {openapiSecurityDefinition} auth: the auth to convert
 ...
 * @param {string} key: the key of the Entry
 * @param {openapiSecurityDefinitionObject} value: the auth object to convert
...
/**
 * converts an openapi Security Definitions Object into a map of Auth Record,
 * taking into account the potential Interface this Auth Record implements.
 * @param {Object<string, Interface>} interfaces: a map of interfaces an Auth Record may implement.
 * @param {openapiSecurityDefinitionsObject} securityDefinitions: a map of security definitions to
 * convert in Auths.
...
/**
 * converts an openapi Security Requirement Object into a map of Interfaces
 * @param {openapiSecurityRequirementObject} security: the security requirement object to convert
...
/**
 * extracts shared Constraints from an openapiDefinitionsObject
 * @param {openapiObject} openapi: the object to extract the definitions from
 * @param {openapiDefinitionsObject} openapi.definitions: the definitions to convert into a
 * TypedStoreInstance
 * @returns {Object<string, Constraint>} the corresponding TypedStoreInstance for constraints
 */
// FIXME components was definitions
methods.getSharedConstraints = ({ components = {} } = {}) => {
  return entries(components.schemas)
 ...
 * @param {openapiResourceObject} entry.value: the resource from which to exptract the tags
...
/**
 * extracts tags from all the operation objects associated with all the resources of an openapi file.
 * @param {openapiObject} openapi: the openapi file from which to get all the resources
 * @param {openapiPathObject} openapi.paths: the object containing all the resources in an openapi
 * file.
 * @returns {Array<string>} an array of string containing all the tags this openapi file contains.
...
/**
 * creates shared tag Interfaces from an openapiObject
 * @param {openapiObject} openapi: the openapi object to extract the shared tags from
 * @returns {Object<string, Interface>} the corresponding TypedStoreInstance for interfaces
 */
methods.getTagInterfaces = (openapi) => {
  const tags = methods.extractTagsFromPathsObject(openapi)
 ...
 * @param {openapiObject} openapi: the openapi file to extract the shared object from.
 * @returns {Store} a store holding all globally shared objects.
 */
methods.getSimpleStore = (openapi = {}) => {
  const sharedEndpoints = methods.getSharedEndpoints(openapi)
  const sharedConstraints = methods.getSharedConstraints(openapi)
  const { sharedParameters, parameterInterfaces } = methods.getSharedParameters(openapi)
  const sharedResponses = methods.getSharedResponses(openapi)
  const authInterfaces = methods.getSharedAuthInterfaces(openapi)
  const sharedAuths = methods.getSharedAuths(authInterfaces, openapi)
  const tagInterfaces = methods.getTagInterfaces(openapi)
 ...
 * @param {Object<string, openapiPathItemObject>} paths: the object to get the structure of.
...
/**
 * creates an Api object based on the openapi object
 * @param {openapiObject} openapi: the openapi object to convert
 * @param {string?} url: the url we got the openapi file from, if we got it from remote
 * @returns {Api} the corresponding Api Record
 */
methods.createApi = (openapi = {}) => {
  const info = methods.getInfo(openapi.info)
  const store = methods.getSimpleStore(openapi)
  const resources = methods.getResources(store, openapi)
  const group = methods.getGroup(openapi)
...
/**
 * converts the content of an openapi file in an Api
 * @param {string} content: the content of the openapi file
 * @param {string} url?: the url of the openapi file, if it was loaded from a remote location
 ...
 * @returns {Api} the Api Record corresponding to the openapi file
 */
methods.parse = ({ options, item: openapi }) => {
  if (!methods.isopenapi(openapi)) {
    return methods.handleInvalidopenapi()
  }

  const api = methods.createApi(openapi)
  return { options, api }
}
